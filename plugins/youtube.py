...
async def ytdl(_, message):
    ...
    try:
        ...
        await message.reply_text("`Невозможно получить данные YouTube... 😔 \nВозможен YouTube заблокирован сервер IP \n#error`")
        return
    buttons = InlineKeyboardMarkup(list(create_buttons(formats)))
    sentm = await message.reply_text("Обработка URL YouTube 🔎 🔎 🔎")
    try:
